 ...
 *
 * Models are loaded once at class initialization so downstream calls can reuse
 * the same detector/tagger instances without repeated disk reads.
 */
public class NLPProcessor {
    ...
    static {
        try {
            ...
            // Sentence model
            InputStream sentenceModelStream = NLPProcessor.class.getResourceAsStream("/models/en-sent.bin");
            ...
            // POS Tagger model
            InputStream posModelStream = NLPProcessor.class.getResourceAsStream("/models/en-pos-maxent.bin");
            POSModel posModel = new POSModel(posModelStream);
            posTagger = new POSTaggerME(posModel);

            // NER model (generic "person" model for now)
            InputStream nerModelStream = NLPProcessor.class.getResourceAsStream("/models/en-ner-person.bin");
