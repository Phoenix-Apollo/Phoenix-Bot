...
- **Sentence Generation**: SentenceGenerator handles conversational phrasing; `Eventlistener` can prepend a signature narrator opener while still delivering factual answers
- **Routing safeguards**:
  - Actionable question-style prompts are routed away from social banter into `AIResponder`
  - `AIResponder` applies deterministic handlers first (identity/status/playful/planning/specific SC picks), then SC/web, then personality fallback
  - Web lookup can be forced for general non-SC question forms when other routes do not answer
...
- `BOT_AI_ENABLED`: Master switch for AI response flow
- `BOT_WEB_LOOKUP_ENABLED`: Enables Wikipedia/DDG lookup path (plus forced question fallback behavior)
- `BOT_AI_SOCIAL_MODE`: Enables social interjections in AI-scoped channels
- `BOT_AI_SOCIAL_REPLY_CHANCE_PERCENT`: Tunes social interjection chance
- `BOT_GIF_REPLIES_ENABLED`: Enables GIF-based social reactions
- `BOT_AUTONOMOUS_LEARNING_ENABLED`: Enables implicit quality-feedback learning
